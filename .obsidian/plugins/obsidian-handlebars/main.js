...
var MyPlugin = class extends import_obsidian.Plugin {
  async onload() {
    ...
    function note_record(file) {
      var _a;
      const { basename, name, path, extension } = file;
      const { frontmatter } = (_a = self_app.metadataCache.getFileCache(file)) != null ? _a : {};
      return { basename, name, path, extension, frontmatter };
    }
    function link_options(path_or_hash) {
      if (!path_or_hash)
        return { path: "" };
      if (typeof path_or_hash === "string") {
        return { path: (0, import_obsidian.normalizePath)(path_or_hash) };
      }
      ...
      return {
        ...
        rel,
        hash,
        alias: alias !== void 0 ? String(alias) : void 0
      };
    }
    Handlebars.registerPartial("link", (path_or_hash, _options) => {
      ...
      if (!(file instanceof import_obsidian.TFile))
        return "";
      return self_app.fileManager.generateMarkdownLink(file, rel ? (0, import_obsidian.normalizePath)(rel) : self_app.vault.getRoot().path, hash, alias);
    });
    Handlebars.registerHelper("notes", (unsafe_path, options) => {
      if (!unsafe_path || typeof unsafe_path !== "string")
        return [];
      const result = self_app.vault.getAbstractFileByPath((0, import_obsidian.normalizePath)(unsafe_path));
      if (!result)
        return [];
      const files = result instanceof import_obsidian.TFolder ? result.children : [result];
      return files.filter((f) => f instanceof import_obsidian.TFile).map((f, index) => {
        var _a;
        return options.fn(note_record(f), { data: { ...(_a = options.data) != null ? _a : {}, index } });
      }).join("");
    });
    const block_handler = (0, import_obsidian.debounce)(async (source, container, { sourcePath: path }) => {
      const file = self_app.vault.getAbstractFileByPath(path);
      try {
        const template = Handlebars.compile(source, { noEscape: true, strict: true });
        await import_obsidian.MarkdownRenderer.renderMarkdown(template(note_record(file)), container, path, void 0);
      } catch (e) {
        console.warn(e);
      }
    }, 250);
    this.registerMarkdownCodeBlockProcessor("handlebars", async (source, el, ctx) => block_handler(source, el, ctx));
  }
};
